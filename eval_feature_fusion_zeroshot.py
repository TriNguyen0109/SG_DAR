#!/usr/bin/env python3
"""
eval_feature_fusion_zeroshot.py — Evaluates B1, B2, B3, B4 models with FEATURE-LEVEL FUSION
using Zero-Shot BLIP or COCO Fine-tuned BLIP.

Modes:
  --mode zs     : PURE Zero-Shot Pretrain (model_base.pth - 129M images, no retrieval fine-tuning)
  --mode zs_14m : PURE Zero-Shot Pretrain (model_base_14M.pth - 14M images)
  --mode cocoft : COCO Retrieval Fine-tuned (model_base_retrieval_coco.pth)
...
class DynamicFeatureFusionEvaluator:
    ...
    def __init__(self, cfg):
        ...
        self.dialog_encoder, self.blip_img = BLIP_BASELINE(
            device=cfg['device'], ckpt=cfg['blip_ckpt']
        )
    ...
    def run(self, hits_at=10):
        ...
        print(f"\n----- {self.cfg['config']} · {self.mode['name']} · Hits@{hits_at} -----")
...
def main():
    p = argparse.ArgumentParser(
        description='Evaluate Feature-Level Fusion using Zero-Shot BLIP (zs) or COCO-FT BLIP (cocoft).',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument('--mode', choices=['zs', 'zs_14m', 'cocoft'], default='zs',
                   help="Retriever mode: 'zs' (Pure 129M Zero-Shot), 'zs_14m' (14M Zero-Shot), or 'cocoft' (COCO Fine-tuned).")
    ...
    p.add_argument('--blip_ckpt', default='',
                   help='Custom URL or local path to checkpoint (overrides --mode).')
    ...
    p.add_argument('--cache_dir', default='',
                   help='Directory to store cached .pth feature vectors (defaults to temp_<mode>).')
    ...
    ckpt = a.blip_ckpt if a.blip_ckpt else BLIP_CKPTS[a.mode]
    mode_tag = a.mode if not a.blip_ckpt else 'custom'
    cache_dir = a.cache_dir if a.cache_dir else f'temp_{mode_tag}'

    base = dict(
        blip_ckpt=ckpt,
        mode_tag=mode_tag,
        queries_path=a.queries_path, corpus_path=a.corpus_path,
        sketch_root=a.sketch_root, cache_dir=cache_dir,
    ...
    todo = list(CONFIGS) if a.config == 'all' else [a.config]
    results = {}

    print(f"🔧 Evaluator Mode: {a.mode.upper()} | Checkpoint: {ckpt}")
    print(f"📁 Cache Directory: {cache_dir}/")

    with torch.no_grad():
        for c in todo:
            cfg = dict(base, config=c)
            cfg['gen_path'] = a.gen_path_dar if c == 'B3' else a.gen_path
            print(f"\n{'='*62}\n{c} — {CONFIGS[c]['name']}\n{'='*62}")
            ev = DynamicFeatureFusionEvaluator(cfg)
            ev.index()
            results[c] = ev.run(hits_at=a.hits_at)

    if len(results) > 1:
        print(f"\n{'='*62}\nFEATURE FUSION SUMMARY ({a.mode.upper()}) — Hits@{a.hits_at} (%)\n{'='*62}")
        print('Turn   ' + ''.join(f'{c:>10}' for c in todo))
        for r in range(a.rounds):
            print(f'{r:>4}   ' + ''.join(f'{results[c][r]:10.2f}' for c in todo))
        if 'B3' in results and 'B4' in results:
            d = results['B4'][-1] - results['B3'][-1]
            print(f'\nB4 − B3 at final turn (R10): {d:+.2f} points (sketch contribution via Feature Fusion)')
